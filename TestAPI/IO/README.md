...
Three standalone programs for the file positions FreeImage carries: `FreeImageIO`'s
`seek_proc` and `tell_proc` take and return `INT64`, `FreeImage_Load()` and
`FreeImage_Save()` seek in 64 bits, and memory streams hold what memory allows.
