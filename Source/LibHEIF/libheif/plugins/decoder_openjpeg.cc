...
struct openjpeg_decoder
{
  ...
  // FreeImage: libheif's thread count, 0 = unspecified
  int num_threads = 0;
};
...
heif_error openjpeg_new_decoder2(void** dec, const heif_decoder_plugin_options* options)
{
  ...
  // FreeImage: keep libheif's thread count
  if (options) {
                                       ...
                                       const heif_security_limits* limits)
{
  ...
  // FreeImage: after opj_setup_decoder(), before opj_read_header(); 0 keeps OPJ_NUM_THREADS
